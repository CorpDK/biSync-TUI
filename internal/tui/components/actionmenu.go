...
const (
	...
	ActionEditMapping   Action = "edit-mapping"
	ActionDeleteMapping Action = "delete-mapping"
)
...
func NewActionMenu(mappingName string, needsInit bool, width, height int) ActionMenuModel {
	...
	if needsInit {
		...
		items = []list.Item{
			...
			ActionItem{action: ActionEditMapping, key: "E", label: "Edit mapping"},
			ActionItem{action: ActionDeleteMapping, key: "X", label: "Delete mapping"},
			ActionItem{action: ActionInfo, key: "i", label: "Info / details"},
			ActionItem{action: ActionRemoteSize, key: "z", label: "Show remote size"},
		}
	}
...
func (m ActionMenuModel) Update(msg tea.Msg) (ActionMenuModel, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case "E":
			return m, m.selectByAction(ActionEditMapping)
		case "X":
			return m, m.selectByAction(ActionDeleteMapping)
		case "b":
